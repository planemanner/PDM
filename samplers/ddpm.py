...
class DDPM(nn.Module):
    ...
    def __init__(self, model, sampler_cfg):
        self.model = model
        self.sampler_cfg = sampler_cfg
        
        if self.sampler_cfg.use_ema:
            self.model_ema = LitEma(self.model)
            print(f"Keeping EMAs of {len(list(self.model_ema.buffers()))}.")

        self.register_schedule(timesteps=self.sampler_cfg.timesteps, 
    ...
    def p_sample_loop(self, shape, return_intermediates=False):
        ...
        intermediates = [img]
        for i in tqdm(range(self.num_timesteps, -1, -1), desc='Sampling t', total=self.num_timesteps):
            img = self.p_sample(img, torch.full((b,), i, device=device, dtype=torch.long),
                                clip_denoised=self.sampler_cfg.clip_denoised)
            if i % self.sampler_cfg.log_every_t == 0 or i == self.num_timesteps - 1:
    ...
    def sample(self, batch_size=16, return_intermediates=False):
        img_size = self.sampler_cfg.image_size
        channels = self.sampler_cfg.in_channels
        return self.p_sample_loop((batch_size, channels, img_size, img_size),
                                  return_intermediates=return_intermediates)
    
    def q_sample(self, x_start: torch.FloatTensor, t: torch.IntTensor, noise: torch.FloatTensor=None):
        # forward process
