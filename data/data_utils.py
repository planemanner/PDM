...
def get_tf_for_sketch(config, mode='train') -> A.BasicTransform:
    ...
    if mode != 'test':
        random_resized_crop = A.RandomResizedCrop(size=(config.transform.resize_height, config.transform.resize_width), scale=(0.8, 1.0))
        ...
        tf = A.Compose([random_resized_crop, hflip, vflip, rot90])
        return tf
    else:
        # test
        tf = A.Compose([resize])
...
def get_tf_for_ae(config, mode='train') -> A.BasicTransform:
    ...
    if mode != 'test':
        ...
        tf = A.Compose([random_resized_crop, 
                        hflip, 
                        vflip, 
                        color_jitter, 
                        rot90])
